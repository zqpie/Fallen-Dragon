...
while True: # game loop
    ...
    #     print("pointgivin")
        ...
        if areObjectsTouching(player, gateway, 50):
            print("Player collided with Gateway")
            roomDone = True
            pen.clear()
            removeObject(player)
            removeObject(wall)
            ...
            with open("data.json", "w") as outfile:
                json.dump(points, outfile)
            os.system('python3 Fallen_Dragon_Room_One.py') # run room one and sent points over
            ...
            #print('python3 Fallen_Dragon_Room_One.py'+str(points))
